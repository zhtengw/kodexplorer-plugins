<?php
	...
	if (isset($_POST['newcontent'])){
	    ...
	    //file_put_contents($localFile,$newcontent, LOCK_EX);
	    $this->pluginCacheFileSet($path, $newcontent);
				...
				if (evt.data.length > 0)
				{
					...
					{					    
					    iframe.contentWindow.postMessage(JSON.stringify({action: 'status',
							    message: "<?php echo LNG('explorer.saving'); ?>", modified: true}),'*');
					    $.ajax({
					        type: 'post',
					        data: {newcontent: msg.xml},
					        success: function(){
					            iframe.contentWindow.postMessage(JSON.stringify({action: 'status',
							    message: "<?php echo LNG('explorer.saved'); ?>", modified: false}),'*');
					        }
					    });
					...
					else if (msg.event == 'save')
					{
					    iframe.contentWindow.postMessage(JSON.stringify({action: 'spinner',
							    message: "<?php echo LNG('explorer.saving'); ?>", show: true, enabled: false}),'*');
					    $.ajax({
					        type: 'post',
					        data: {newcontent: msg.xml},
					        success: function(){
					            ...
					            iframe.contentWindow.postMessage(JSON.stringify({action: 'spinner',
							    message: "<?php echo LNG('explorer.saved'); ?>", show: true}),'*');
